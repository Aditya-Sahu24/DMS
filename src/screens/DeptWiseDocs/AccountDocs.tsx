...
const AccountDocs = ({ navigation }: { navigation: any }) => {
    ...
    try {
      const response = await api.post(
        "DocMangr/GetDocMangr",
        {
          docMid: -1,
          fileTypId: -1,
          divisionid: 6, // Keep the divisionid as 6 for accounts documents
          subsubjId: -1,
          user_Id: -1,
          fileNo: "",
        }
  ...
  return (
    <>
      ...
      <View style={styles.container}>
        <Text style={styles.title}>Accounts Department Documents</Text>
        <TextInput
              ...
              {filteredData.map((item, index) => (
                <View key={item.docMid}>
                  ...
                  >
                    ...
                    <Text
                      style={[styles.cell, { width: columnWidths.fileNo }]}
                    >
                      {item.fileNo}
                    </Text>
                    <Text
                      style={[styles.cell, { width: columnWidths.fileDef }]}
                    >
                      {item.fileDef || "-"}
                    </Text>
                    <Text
                      style={[styles.cell, { width: columnWidths.department }]}
                    >
                    ...
                    <Text
                      style={[styles.cell, { width: columnWidths.fileCount }]}
                    >
                  ...
                  {expanded === item.docMid && (
                    <View style={styles.detailsContainer}>
                      <Text style={styles.detailsTitle}>
                        Details for Document #{item.docMid}
                      </Text>
                      <Text>
                        <Text style={styles.bold}>Synopsis:</Text>{" "}
                        {item.synopsis}
                      </Text>
                      <Text>
                      ...
                      <Text>
                        <Text style={styles.bold}>Auth Person:</Text>{" "}
                        {item.authPersn || "-"}
                      </Text>
                      <Text>
                        <Text style={styles.bold}>Sub-Subject Name:</Text>{" "}
                        {item.sub_Subject_Name}
                      </Text>
                      <Text>
                        <Text style={styles.bold}>File Type Name:</Text>{" "}
                        {item.fileType_Name}
                      </Text>
                      <Text>
                        <Text style={styles.bold}>Period From:</Text>{" "}
                        {formatDate(item.perFr)}
                      </Text>
                      <Text>
                        <Text style={styles.bold}>Period Until:</Text>{" "}
                        {formatDate(item.perUt)}
                      </Text>
                      <Text>
                        <Text style={styles.bold}>Create Date:</Text>{" "}
                        {formatDate(item.createdate)}
                      </Text>

                      <Text style={styles.subHeader}>
                        Associated Files ({item.member_DocFiles?.length})
                      </Text>
                      {item.member_DocFiles.map((file) => (
                        <View key={file.pdFid} style={styles.fileBox}>
                          <Text style={styles.bold}>
                            {file.pdfName} (ID: {file.pdFid})
                          </Text>
